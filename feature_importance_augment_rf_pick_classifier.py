...
def process_csv_files(filename, augment=False, augment_pct=0.05, rng=None):
    ...
    features = np.column_stack((flex_arr_new, p_arr_new, filtered_force, tof_arr_new)) # all sensors
...
# ---------- Bag-level loading ----------
def load_bags(directory, augment_failed=False, augment_success=False,
              num_aug_failed=5, num_aug_success=1, augment_pct=0.05):
    """
    Load all bags and optionally augment failed or success bags.
    """
    bags = []
    for file in os.listdir(directory):
        if file.endswith(".csv") and ".db3force" in file:
            ...
            # ---- Augment failed bags ----
            if augment_failed:
                for k in range(num_aug_failed):
                    seed = 12 + k  # deterministic per copy
                    rng = np.random.default_rng(seed)
                    features_aug, labels_aug = process_csv_files(
                        prefix, augment=True, augment_pct=augment_pct, rng=rng
                    )
                    bags.append((prefix, features_aug, labels_aug))

            # ---- Augment success bags ----
            if augment_success:
                for k in range(num_aug_success):
                    seed = 42 + k  # deterministic per copy
                    rng = np.random.default_rng(seed)
                    features_aug, labels_aug = process_csv_files(
                        prefix, augment=True, augment_pct=augment_pct, rng=rng
                    )
...
# ----------- Plotting ----------
def plot_bag_file(filename, split="UNKNOWN", clf=None, window_size=WINDOW_SIZE, step=STEP_SIZE, save_pdf=None, scaler=None):
    # ---- Load signals ----
    ...
    if clf:
        ...
        if scaler is not None:
            X_windowed = scaler.transform(X_windowed)
...
if __name__ == "__main__":
    ...
    # 11 failed bags → 13 augmentations each → 143 failure bags
    failed_bags  = sorted(load_bags(failed_dir, augment_failed=True, num_aug_failed=7), key=lambda x: x[0])

    # 72 success bags → 1 augmentation each → 144 success bags total
    success_bags = sorted(load_bags(success_dir, augment_success=True, num_aug_success=1), key=lambda x: x[0])


    all_bags = failed_bags + success_bags
    print(f"ALL BAGS: {len(all_bags)}")
    ...
    # --- 1 Define sensor and feature labels ---
    sensor_names = ["Flex", "Pressure", "Force", "TOF"]
    n_sensors = len(sensor_names)
    n_timesteps = WINDOW_SIZE

    # feature labels for flattened windows
    feature_labels = []
    for sensor in sensor_names:
        for t in range(n_timesteps):
            feature_labels.append(f"{sensor}_t{t}")

    # --- 2 Overall RF feature importance ---
    ...
    # Sum importance per sensor across timesteps
    sensor_importance = feat_df.groupby(feat_df['feature'].str.split('_').str[0])['importance'].sum()
    print("\n=== Overall sensor importance ===")
    print(sensor_importance.sort_values(ascending=False))

    # --- 3 Feature importance per state (summed over timesteps) ---
    ...
    for state, name in zip(states_to_check, state_names):
        ...
        r = permutation_importance(clf, X_state, y_state, n_repeats=10, random_state=42)
        perm_importances = r.importances_mean

        # Sum over timesteps for each sensor
        sensor_importances = []
        for i in range(n_sensors):
            sensor_importances.append(np.sum(perm_importances[i*n_timesteps:(i+1)*n_timesteps]))
        state_sensor_importance[name] = sensor_importances
